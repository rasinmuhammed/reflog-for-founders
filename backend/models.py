...
class Competitor(Base):
    """
    Tracks competitors for strategic market intelligence.
    Uses public data sources only (Product Hunt, Twitter, blogs, etc.)
    """
    ...
    # Basic info
    name = Column(String(255))  # e.g., "Linear", "Notion"
    website = Column(String(500))
    category = Column(String(100))  # "Project Management", "CRM", etc.
    notes = Column(Text, nullable=True)

    # Public tracking sources (user-provided)
    product_hunt_url = Column(String(500), nullable=True)
    twitter_handle = Column(String(100), nullable=True)
    blog_rss = Column(String(500), nullable=True)
    ...
    # Indexes for fast queries
    __table_args__ = (
        Index('idx_competitor_user_active', 'user_id', 'is_active'),
    )


class CompetitorIntel(Base):
    """
    Stores AI-generated competitive intelligence reports.
    Generated by CompetitorIntelligenceCrew using Scout + Analyst + Strategist agents.
    """
    ...
    raw_data = Column(JSON, nullable=True)  # Structured research data with sources

    # Change detection (from Analyst agent)
    changes_detected = Column(JSON, nullable=True)  # What changed since last check

    # Strategic synthesis (from Strategist agent)
    competitive_brief = Column(Text)  # Full intelligence brief
    threat_level = Column(String(20))  # "low", "medium", "high", "critical"
    recommended_actions = Column(JSON, nullable=True)  # Specific action items

    # Specific insights
    pricing_changes = Column(JSON, nullable=True)
    feature_updates = Column(JSON, nullable=True)
    market_positioning = Column(Text, nullable=True)

    # Indexes for performance
    __table_args__ = (
        Index('idx_intel_competitor_date', 'competitor_id', 'report_date'),
    )
